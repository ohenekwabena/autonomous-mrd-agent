...
async def validation_handler_main(ctx: OrchestratorContext) -> OrchestratorContext:
    ...
    if ctx.validation_result:
        ...
        # Derive overall confidence from completeness + validation quality
        completeness = ctx.research_aggregate.data_completeness_score or ctx.research_aggregate.calculate_completeness()
        errors = len(result.errors)
        warnings = len(result.warnings)

        if errors > 0:
            derived_conf = ConfidenceLevel.LOW
        elif completeness >= 0.9 and warnings <= 2:
            derived_conf = ConfidenceLevel.HIGH
        elif completeness >= 0.7:
            derived_conf = ConfidenceLevel.MEDIUM
        elif completeness >= 0.5:
            derived_conf = ConfidenceLevel.LOW
        else:
            derived_conf = ConfidenceLevel.UNVERIFIED

        # Apply derived confidence if MRD exists
        if ctx.mrd:
            ctx.mrd.overall_confidence = derived_conf
            print(f"  • Derived overall confidence: {derived_conf.value} (completeness: {completeness:.1%})")
...
def save_mrd_output(result: OrchestratorContext, output_dir: str = "output"):
    ...
    def fmt_sources(citations):
        if not citations:
            return "unspecified"
        return ", ".join(fmt_enum(c.source) for c in citations)
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"**Confidence:** {fmt_enum(result.mrd.overall_confidence)}\n\n")
        
        f.write(f"## Executive Summary\n\n{result.mrd.executive_summary}\n\n")
        
        f.write(f"## Market Overview\n\n")
        for claim in result.mrd.market_overview[:3]:  # First 3 claims
            sources = fmt_sources(claim.citations)
            f.write(f"- **{claim.claim}** (Confidence: {fmt_enum(claim.confidence)}, Sources: {sources})\n")
        ...
        def write_claims(title: str, claims: list[VerifiedClaim]):
            ...
            for claim in claims[:5]:
                conf = fmt_enum(claim.confidence)
                sources = fmt_sources(claim.citations)
        ...
        for feat in result.mrd.feature_recommendations:
            priority = fmt_enum(feat.priority)
            effort = fmt_enum(feat.estimated_effort)
        ...
        for reg in result.mrd.regulatory_analysis:
            status = fmt_enum(reg.legal_status)
